...
class AVContinualDataset:
    ...
    def store_masked_loaders(self, train_dataset: datasets, test_dataset: datasets) -> Tuple[DataLoader, DataLoader]:
        """
        ...
        test_mask = np.logical_and(np.array(test_dataset.targets) >= self.i,
                                   np.array(test_dataset.targets) < self.i + self.N_CLASSES_PER_TASK)

        train_dataset.video = train_dataset.video[train_mask]
        train_dataset.audio = train_dataset.audio[train_mask]

        test_dataset.video = test_dataset.video[test_mask]
        test_dataset.audio = test_dataset.audio[test_mask]

        train_dataset.targets = np.array(train_dataset.targets)[train_mask]
        test_dataset.targets = np.array(test_dataset.targets)[test_mask]

        print(f'Task {self.task} Training Samples:', len(train_dataset.video))
        print(f'Task {self.task} Test Samples:', len(test_dataset.video))
    ...
    def store_loaders(self, train_dataset: datasets, test_dataset: datasets) -> Tuple[DataLoader, DataLoader]:
        """
        Divides the dataset into tasks.
        :param train_dataset: train dataset
        :param test_dataset: test dataset
...
def get_previous_train_loader(train_dataset: datasets, batch_size: int,
                              setting: AVContinualDataset) -> DataLoader:
    """

...
class VGGSound_dataset(object):
    def __init__(self, path_to_dataset = '/data/input-ai/datasets/VGGSound/data/vggsound/data_processed/', frame_interval=1, frame_kept_per_second=1):
        self.path_to_video = os.path.join(path_to_dataset, 'video')
        self.frame_kept_per_second = frame_kept_per_second
        self.path_to_save = os.path.join(path_to_dataset, 'images', 'Image-{:02d}-FPS'.format(self.frame_kept_per_second))
        if not os.path.exists(self.path_to_save):
            os.mkdir(self.path_to_save)

        self.file_list = os.listdir(self.path_to_video)
